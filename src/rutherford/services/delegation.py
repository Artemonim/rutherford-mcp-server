...
class DelegationService:
    ...
    async def _delegate_body(  # noqa: C901 - the guard ladder every delegation passes through, in order
        self,
        req: DelegationRequest,
        *,
        correlation_id: str,
    ...
    async def _run_direct(
        ...
        *,
        ...
        and after a SUCCESSFUL turn; a change fails the result with ``READONLY_VIOLATED`` -- the agent's
        read-only promise made a checked invariant rather than a trusted one. Skipped for a mutating mode,
        which is expected to change the tree.
