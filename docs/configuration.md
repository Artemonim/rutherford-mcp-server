...
| `trusted_workspaces` | `list[str]` | `[]` | Absolute paths under which `write` / `yolo` delegations are permitted. Resolved to absolute; a missing directory warns. Edit the global list from a terminal with `rutherford trust` / `untrust` (or `rutherford-mcp-server` / `python -m rutherford`; cwd or an explicit path); `trust --list` prints it. Read once at server start, so restart after a change. |
